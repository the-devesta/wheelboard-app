...
import 'package:wheelboard/controllers/Professional/new_referral_screen_controller.dart';

class NewReferralScreen extends StatelessWidget {
  NewReferralScreen({super.key});

  final controller = Get.put(NewReferralController());
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
        child: GetBuilder<NewReferralController>(
          builder: (_) {
            return Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ...
                Wrap(
                  ...
                  children: controller.roles.map((role) {
                    ...
                    return ChoiceChip(
                      ...
                      onSelected: (_) =>
                          controller.selectRole(role["title"].toString()),
                    );
                  }).toList(),
                ),
                ...
                Row(
                  ...
                  children: [
                    Obx(
                      () => Checkbox(
                        value: controller.notify.value,
                        activeColor: Colors.redAccent,
                        onChanged: (v) => controller.toggleNotify(v ?? false),
                      ),
                    ),
                    const Expanded(
                      child: Column(
