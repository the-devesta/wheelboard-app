...
//         '?input=$input&types=establishment|geocode&language=en&components=country:in&key=$apiKey';
...
//       if (result['status'] == 'OK') {
//         return (result['predictions'] as List).map<Suggestion>((p) {
//           final terms = (p['terms'] as List).cast<Map<String, dynamic>>();

//           final sector = getTerm(terms, 4);
//           final city = getTerm(terms, 3);
//           final state = getTerm(terms, 2);
//           final country = getTerm(terms, 1);
//           final parts = [
...
//         return [];
//       } else {
//         throw Exception(result['error_message'] ?? 'Unknown API error');
//       }
//     } else {
